...
Filtar musicas por compositores

Tendo em conta a ordem dos dados no ficheiro, podemos chegar ao compositor atraves da localização da data
- `\d{4};`: encontra o padrão de 4 dígitos seguidos, útil para o ano associado a cada entrada.
- `[^;]*;`: encontra qualquer caracter diferente de `;`. Permite detear o periodo 
- `([^;]*)`:  padrão associado ao atributo do nome 

Apos adicionar os matchs a uma lista, uso um `.sort`

#### 2.

Contar musica por época

Aproveitando a expressão regular anterior, podemos contar o nr de musicas de cada epoca com um dicionario

#### 3.

Nome das musicas por epoca

Tendo em conta a informação pedida, precisamos de detetar dois padroes distintos, que podem ser expressos na dijunção seguinte
`(((\d\d\d\d;)([^;]*);.*$)|^([^;\n]*);[^ \w])`.


- `((\d\d\d\d;)([^;]*);.*$)`:  período a partir do fim da linha
- `^([^;\n]*);[^ \w])`: nome da obra
